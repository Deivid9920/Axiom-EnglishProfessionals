...
// E2E de Axiom (inglés): autenticación, navegación de módulos y RBAC.
...
test('navegación al módulo de Escritura', async ({ page }) => {
  await skipOnboarding(page)
  await login(page, DEMO_EMAIL, DEMO_PASSWORD)
  await page.locator('.sidebar-link', { hasText: 'Escritura' }).click()
  await expect(page.getByText('Mecanografía').first()).toBeVisible({ timeout: 30000 })
})
